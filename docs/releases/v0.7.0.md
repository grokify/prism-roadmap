...
### Pandoc PDF Unicode Support

YAML frontmatter now uses DejaVu fonts with `fontspec` for full Unicode/emoji support in PDF output:

```bash
# Generate PDF with XeLaTeX (required for Unicode support)
pandoc output.md -o output.pdf --pdf-engine=xelatex
```

This fixes errors like `LaTeX Error: Unicode character ✅ (U+2705) not set up for use with LaTeX`.
